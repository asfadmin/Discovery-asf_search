...
def test_ASFProduct_Geojson_EmptyResponse() -> None:
    apiResponse = {"geometry": {}, "properties": {}}
    product = ASFProduct(apiResponse)
...
def test_ASFProduct_Geo_Search(basic_response):
    resp = basic_response
    product = ASFProduct(resp)

    geojson = product.geojson()
    assert(geojson['geometry'] == resp['geometry'])
    assert(geojson['properties'] == resp['properties'])

@pytest.fixture
def basic_response() -> Dict[str, Dict]:
    return {
        "geometry": {
            "coordinates": [
            [
