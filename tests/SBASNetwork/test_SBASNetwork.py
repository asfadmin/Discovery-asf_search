...
from asf_search import SBASNetwork, S1MultiBurstSBASNetwork, S1MultiBurstGroup, get_pair_from_dates
...
def test_add_pair(sbas_network):
    removed_pair_0 = get_pair_from_dates(sbas_network.remove_list, date(2023,1,17), date(2024,6,16))
...
def test_add_pairs(sbas_network):
    removed_pair_0 = get_pair_from_dates(sbas_network.remove_list, date(2023,1,17), date(2024,6,16))
    removed_pair_1 = get_pair_from_dates(sbas_network.remove_list, date(2023,1,5), date(2024,2,5))
...
def test_remove_pairs(sbas_network):
    pair1_to_remove = get_pair_from_dates(sbas_network.subset_stack, date(2025,1,6), date(2025,1,18))
    pair2_to_remove = get_pair_from_dates(sbas_network.subset_stack, date(2025,1,6), date(2025,1,30))
